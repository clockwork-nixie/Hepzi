...
namespace Hepzi.Application.Sessions
{
    public class SessionActions : IDisposable
    {
        ...
        private SessionAction _current = new(Array.Empty<byte>(), null, false, null);
        ...
        {
            public SessionAction(byte[] buffer, int? targetUserId, bool isTerminal, int? excludeUserId)
            {
                Buffer = buffer;
        ...
        public void AddAction(byte[] buffer, int? targetUserId = null, bool isTerminal = false, int? excludeUserId = null)
        {
            lock (_lock)
            {
                var action = new SessionAction(buffer, targetUserId, isTerminal, excludeUserId);
